...
import { CheckCheck, Search, Plus, ExternalLink } from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500 font-normal mt-1">
            History of all emails sent successfully by the background worker.
          </p>
        </div>
      ...
      {sentEmails.length === 0 ? (
        <div className="bg-white border border-neutral-200/90 rounded-2xl p-10 text-center shadow-2xs">
          <div className="w-12 h-12 rounded-full bg-neutral-100 flex items-center justify-center text-neutral-500 mx-auto mb-3">
            <CheckCheck className="w-6 h-6 stroke-[1.8]" />
          </div>
          <h3 className="text-base font-normal text-neutral-900 mb-1">
            {searchQuery ? 'No matching sent emails' : 'No sent emails yet'}
          </h3>
          <p className="text-xs sm:text-sm text-neutral-500 max-w-sm mx-auto mb-5 font-normal">
            {searchQuery
              ? 'Try changing your search terms.'
              : 'Emails will appear here once their scheduled time arrives and they are delivered.'}
          </p>
          {!searchQuery && (
            <button
              onClick={onComposeClick}
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2.5 flex-wrap">
                  ...
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-100 text-xs font-normal">
                ...
                <div className="flex items-center gap-2 text-xs text-neutral-500 mt-1.5 flex-wrap font-normal">
                  ...
                  <span className="text-neutral-600">
                    Delivered at {formatSentTime(email.sentAt || email.updatedAt)}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 shrink-0 self-end sm:self-center pt-2 sm:pt-0">
                {email.previewUrl && (
                  <a
                    ...
                    className="inline-flex items-center gap-1 text-sm font-normal text-neutral-700 hover:text-neutral-950 px-2 py-1 transition-colors cursor-pointer"
                    title="View email on Ethereal test inbox"
                  >
                    <span>Inbox</span>
